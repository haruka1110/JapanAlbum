...
<script>
        ...
        function showOverlay(imageUrl, photoId) {
            ...
            deleteButton.href = "<%=request.getContextPath()%>/Gunmadelete?id=" + photoId;
        }
...
<body>
	...
	<h3>写真を追加する</h3>
	<form action="<%=request.getContextPath()%>/Gunmaadd" method="get">
		<input type="submit" value="写真を追加する">
	</form>
	<form action="<%=request.getContextPath()%>/Gunma" method="get">
	...
	<ul>
		...
		for (Photo photo : photoList) {
			...
			// 関東地方の県のみ表示する条件を追加
			if (photo.getLocation().equals("群馬県")) {
		%>
		<li>
			<div>
	...
	<div id="overlay" onclick="hideOverlay()">
		<img src="" alt="写真">
		<form action="<%=request.getContextPath()%>/Gunmadelete" method="post">
